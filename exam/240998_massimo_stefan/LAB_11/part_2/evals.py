...
def test_tag_functions():
    # Mock data
    ote_tag_sequence = ['B', 'E', 'O', 'O']
    ts_tag_sequence = ['B-POS', 'E-POS', 'O', 'O']

    print("Results for tag2ot:", tag2ot(ote_tag_sequence))
    print("Expected: [(0, 1)]\n")

    print("Results for tag2ts:", tag2ts(ts_tag_sequence))
    print("Expected: [(0, 1, 'POS')]")
    
def test_match_functions():
    # Mock data
    gold_ote_sequence = [(0, 1)]
    pred_ote_sequence = [(0, 1)]

    gold_ts_sequence = [(0, 1, 'POS')]
    pred_ts_sequence = [(0, 1, 'POS')]

    print("Results for match_ot:", match_ot(gold_ote_sequence, pred_ote_sequence))
    print("Expected: 1\n")

    hit_count, gold_count, pred_count = match_ts(gold_ts_sequence, pred_ts_sequence)
    print("Results for match_ts - Hit count:", hit_count)
    print("Results for match_ts - Gold count:", gold_count)
    print("Results for match_ts - Pred count:", pred_count)
    print("Expected: [1. 0. 0.]\n")

def test_eval_function():
    # Mock data
    gold_ot = [['B', 'E', 'O', 'O'], ['B', 'E', 'O', 'O']]
    pred_ot = [['B', 'E', 'O', 'O'], ['B', 'E', 'O', 'O']]

    gold_ts = [['B-POS', 'E-POS', 'O', 'O'], ['B-NEG', 'E-NEG', 'O', 'O']]
    pred_ts = [['B-POS', 'E-POS', 'O', 'O'], ['B-NEG', 'E-NEG', 'O', 'O']]

    # Let's call the evaluate function
    ote_scores, ts_scores = evaluate(gold_ot, gold_ts, pred_ot, pred_ts)
    
    ote_precision, ote_recall, ote_f1 = ote_scores
    ts_precision, ts_recall, ts_f1 = ts_scores

    print("OTE Precision:", ote_precision)
    print("OTE Recall:", ote_recall)
    print("OTE F1:", ote_f1)

    print("\nTS Precision:", ts_precision)
    print("TS Recall:", ts_recall)
    print("TS F1:", ts_f1)
