...
if __name__ == "__main__":
    ...
    #k_fold_evaluation(criterion, tokenizer, sub_sentences, sub_labels, sub_model)
    
    mr_documents, mr_labels = get_mr_doc()
    
    print('Polarity task: full dataset')
    k_fold_evaluation(criterion, tokenizer, mr_documents, mr_labels, 'pol_bert_full')
    
    mr_documents_subj, mr_labels_subj = filter_subj_doc(mr_documents, tokenizer, sub_model)
    
    print('Polarity task: subjective dataset')
    k_fold_evaluation(criterion, tokenizer, mr_documents, mr_labels, 'pol_bert_subj')